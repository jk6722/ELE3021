...
forkthreadmain(void *arg)
{
  int pid;
  if ((pid = fork()) == -1){
    printf(1, "panic: fork in forktest\n");
    exit();
  } else if (pid == 0){
    printf(1, "child\n");
    exit();
    ...
    printf(1, "parent\n");
    if (wait() == -1){
      printf(1, "panic:  wait in forktest\n");
      exit();
    }
  }
  thread_exit(0);
...
pipetest(void)
{
  ...
  if (pipe(fd) < 0){
    printf(1, "panic:  pipe in pipetest\n");
    return -1;
  }
  arg[1] = fd[0];
  arg[2] = fd[1];
  if ((pid = fork()) < 0){
      printf(1, "panic:  fork in pipetest\n");
      return -1;
  } else if (pid == 0){
    close(fd[0]);
    arg[0] = 0;
  ...
  if (wait() == -1){
    printf(1, "panic:  wait in pipetest\n");
    return -1;
  }
  if (gcnt != 0)
    printf(1,"panic:  validation in pipetest : %d\n", gcnt);
